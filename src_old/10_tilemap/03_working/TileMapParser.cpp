...
TileMapParser::TileMapParser(/* WorkingDirectory& workingDir, */ ResourceAllocator<sf::Texture> &textureAllocator) : /*  workingDir(workingDir),  */ textureAllocator(textureAllocator) {}
...
std::vector<std::shared_ptr<Object>> TileMapParser::Parse(const std::string &file, sf::Vector2i offset)
{
    ...
    // #else
    //     strcpy_s(fileLoc, file.c_str(), file.size() + 1);
    // #endif

    //TODO: error checking - check file exists before attempting open.
    rapidxml::file<> xmlFile(&file.c_str()[0]);
    ...
    for (const auto &layer : *tiles)
    {
        for (const auto &tile : *layer.second)
        {
            ...
            float x = tile->x * tileSizeX + offset.x;
            float y = tile->y * tileSizeY + offset.y;
...
std::shared_ptr<TileSheetData> TileMapParser::BuildTileSheetData(xml_node<> *rootNode)
{
    ...
    tileSheetData.textureId = textureAllocator.Add(/* workingDir.Resources() + */ std::string(imageNode->first_attribute("source")->value()));
